...
export default function OilMethodologyTabs({ view, oilChangePct, gasolineEffectPct, directEffectPp, expandedEnergy, stressScenario, durationImpact, componentEffects, oilMacroMonthly = [], localProjection, selectedMetro }: OilMethodologyTabsProps) {
  ...
  if (view === "duration") {
    return (
      <section className="grid min-w-0 gap-5 rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
        <div>
          <h2 className="break-words text-lg font-black leading-7 text-slate-950 sm:text-xl">Calibrated duration model</h2>
        ...
        <div className="grid min-w-0 gap-4 md:grid-cols-2 xl:grid-cols-4">
          <div className="min-w-0 rounded-xl bg-blue-50 p-4"><p className="text-sm font-bold text-blue-700">Cumulative CPI pressure range</p><p className="mt-2 break-words text-2xl font-black leading-tight text-blue-950">+{durationImpact.cumulativePressureLowPct.toFixed(2)}% to +{durationImpact.cumulativePressureHighPct.toFixed(2)}%</p></div>
          <div className="min-w-0 rounded-xl bg-green-50 p-4"><p className="text-sm font-bold text-green-700">Average monthly pressure range</p><p className="mt-2 break-words text-2xl font-black leading-tight text-green-950">+{durationImpact.monthlyPressureLowPct.toFixed(2)}% to +{durationImpact.monthlyPressureHighPct.toFixed(2)}%</p></div>
          <div className="min-w-0 rounded-xl bg-slate-50 p-4"><p className="text-sm font-bold text-slate-700">Energy channel contribution</p><p className="mt-2 break-words text-2xl font-black leading-tight text-slate-950">+{durationImpact.energyChannelContributionPp.toFixed(2)}%</p></div>
          <div className="min-w-0 rounded-xl bg-amber-50 p-4"><p className="text-sm font-bold text-amber-700">Indirect channel contribution</p><p className="mt-2 break-words text-2xl font-black leading-tight text-amber-950">+{durationImpact.indirectChannelContributionPp.toFixed(2)}%</p></div>
        </div>
        <div className="grid min-w-0 gap-3 break-words text-sm sm:grid-cols-2 lg:grid-cols-4">
  ...
  if (view === "econometric") {
    return (
      <div className="min-w-0 rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
        <h2 className="break-words text-lg font-black leading-7 text-slate-950 sm:text-xl">Econometric model</h2>
  ...
  if (view === "ml") {
    return (
      <div className="min-w-0 rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
        <h2 className="break-words text-lg font-black leading-7 text-slate-950 sm:text-xl">ML experimental</h2>
  ...
  if (view === "simple") {
    return (
      <div className="grid min-w-0 gap-6">
        <section className="min-w-0 rounded-xl border border-blue-100 bg-blue-50 p-4 sm:p-6">
          <h2 className="break-words text-lg font-black leading-7 text-blue-950 sm:text-xl">Plain-English explanation</h2>
